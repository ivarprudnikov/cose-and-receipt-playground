...
func main() {
	...
	http.Handle("GET /.well-known/did.json", pre(didHandler()))
	http.Handle("POST /signature/create", pre(sigCreateHandler()))
	http.Handle("POST /signature/verify", pre(sigVerifyHandler()))
	http.Handle("POST /receipt/create", pre(receiptCreateHandler()))
	http.Handle("POST /receipt/verify", pre(receiptVerifyHandler()))
	http.Handle("GET /", pre(indexHandler()))
	http.Handle("GET /index.html", pre(indexHandler()))
	http.Handle("GET /favicon.ico", pre(faviconHandler()))
	port := getPort()
